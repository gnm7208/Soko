import { ArrowRight, Loader2, Package, Plus, Settings, Star, Store, TrendingUp, Wallet } from "lucide-react";
import { useEffect, useState } from "react";
import { toast } from "sonner";

import { Button } from "@/components/ui/button";

import { api, type ApiProfile, type ApiWallet } from "@/services/api";
...
import { normalizeShop } from "./normalizers";
...
export function SellerDashboard({ searchQuery, onSwitchToBuyer, profile, listings = fallbackListings, shops = fallbackShops, orders = fallbackOrders, onShopUpdated }: SellerDashboardProps) {
  const isRetailer = profile?.role === "retailer";
  const [ownShop, setOwnShop] = useState<Shop | null>(null);
  const [ownShopLoading, setOwnShopLoading] = useState(isRetailer);
  const [settingsOpen, setSettingsOpen] = useState(false);

  const refreshOwnShop = () => {
    if (!isRetailer) { setOwnShopLoading(false); return; }
    setOwnShopLoading(true);
    api
      .getMyShop()
      .then((raw) => setOwnShop(normalizeShop(raw)))
      .catch(() => setOwnShop(null))
      .finally(() => setOwnShopLoading(false));
  };

  useEffect(refreshOwnShop, [isRetailer, profile?.id]);

  // Anonymous/non-retailer visits (reached via the ungated Buy/Sell toggle) keep the
  // demo preview behaviour; authenticated retailers only ever see their own shop.
  const shop = isRetailer ? ownShop : (shops[0] ?? fallbackShops[0]);
  const initialListings = shop ? listings.filter((listing) => listing.shopId === shop.id) : [];
  ...
  useEffect(() => {
    if (!shop) { setSellerListings([]); return; }
    const nextListings = listings.filter((listing) => listing.shopId === shop.id);
    setSellerListings(nextListings.map((listing) => ({ ...listing, status: "Active" })));
  }, [listings, shop]);
  ...
  if (isRetailer && !ownShop) {
    return (
      <div className="mx-auto max-w-2xl px-4 py-16 text-center sm:px-5">
        <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/15 text-primary"><Store className="h-6 w-6" aria-hidden="true" /></div>
        <h1 className="font-heading text-2xl font-bold tracking-[-.03em]">Create your shop</h1>
        <p className="mt-2 font-body text-sm text-muted-foreground">You don’t have a shop yet — set one up to start listing products and taking orders.</p>
        <Button type="button" className="mt-5 gap-2 rounded-lg font-heading text-sm" onClick={() => setSettingsOpen(true)}><Plus className="h-4 w-4" aria-hidden="true" /> Create your shop</Button>
        <ShopSettingsDialog
          open={settingsOpen}
          onOpenChange={setSettingsOpen}
          onUpdated={() => undefined}
          onCreated={() => { toast.success("Shop created — pending approval"); refreshOwnShop(); onShopUpdated?.(); }}
        />
      </div>
    );
  }
