...
`AuthService.delete_account` (server/services/auth_service.py) is the single place that knows what
a profile owns. It refuses (409) while any order involving the person is in `pending`, `confirmed`,
`paid`, `preparing` or `out_for_delivery`, or their wallet balance is positive, and deletes
dependents explicitly because SQLite does not enforce `ON DELETE CASCADE`. If you add a table
that references `profiles.id`, add it there and to `server/tests/test_account_deletion.py`.
Wrong password answers 403, never 401 — the UI signs out on 401.
